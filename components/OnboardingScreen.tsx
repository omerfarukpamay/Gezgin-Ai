...
export const OnboardingScreen: React.FC<OnboardingScreenProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    if (startDate && endDate) {
      ...
      setPrefs(prev => ({ ...prev, dates: `${startStr} - ${endStr}`, duration: diffDays }));
    } else {
       setPrefs(prev => ({ ...prev, dates: '', duration: 1 }));
    }
  }, [startDate, endDate]);
            ...
            <div className="flex-1 animate-fade-in flex flex-col">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input type="text" value={prefs.city} onChange={e => setPrefs({...prefs, city: e.target.value})} placeholder="Enter city name" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none bg-gray-50" />
                </div>
                <div className="flex-1 flex flex-col">
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <div className="font-bold text-gray-800 text-lg">{MONTHS[currentDate.getMonth()]} {currentDate.getFullYear()}</div>
                    ...
                    <div className="grid grid-cols-7 gap-1">
                      {Array(getFirstDayOfMonth(currentDate.getFullYear(), currentDate.getMonth())).fill(null).map((_, i) => <div key={`blank-${i}`} className="h-9" />)}
                      {Array.from({ length: getDaysInMonth(currentDate.getFullYear(), currentDate.getMonth()) }, (_, i) => i + 1).map(day => {
                        ...
                        if (isPast) classes += "text-gray-300 cursor-not-allowed bg-gray-50/50 ";
                        else {
                  ...
                  <div className="mt-3 text-center h-5">
                    {startDate && endDate ? <span className="text-xs font-semibold text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full">{startDate.toLocaleDateString('en-US', {day:'numeric', month:'short'})} - {endDate.toLocaleDateString('en-US', {day:'numeric', month:'short'})}</span> : <span className="text-xs text-gray-400">Select Start and End dates</span>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col animate-fade-in relative touch-none">
                    ...
                    if (!isCurrent) {
                      return (
                        <div key={card.id} className={`${wrapperClass} scale-95 translate-y-4 opacity-50 transition-all duration-300`}>
                           ...
                           <div className="p-6"><h3 className="text-xl font-bold text-gray-800">{card.title}</h3><p className="text-sm text-gray-500 mt-2">{card.description}</p></div>
                        </div>
                      )
                    }
                    return (
                      <div key={card.id} ref={cardRef} className={`${wrapperClass} transition-transform duration-300 ease-out`} style={getCardStyle(index)} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerLeave={handlePointerUp}>
                         <div className="relative h-3/5 w-full">
                           <img src={card.image} className="w-full h-full object-cover pointer-events-none" alt={card.title} />
                           {dragStart && dragCurrent && (dragCurrent.x - dragStart.x) > 50 && <div className="absolute top-4 left-4 bg-green-500 text-white px-4 py-1 rounded-lg font-bold transform -rotate-12 border-2 border-white shadow-lg">YES</div>}
                           {dragStart && dragCurrent && (dragCurrent.x - dragStart.x) < -50 && <div className="absolute top-4 right-4 bg-red-500 text-white px-4 py-1 rounded-lg font-bold transform rotate-12 border-2 border-white shadow-lg">NO</div>}
                         </div>
                         <div className="p-6"><h3 className="text-xl font-bold text-gray-800">{card.title}</h3><p className="text-sm text-gray-500 mt-2">{card.description}</p><div className="mt-4 flex gap-2"><span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-md font-medium">#{card.tag}</span></div></div>
                      </div>
                    );
                  })}
              </div>
              {currentCardIndex < INTEREST_CARDS.length && (
                <div className="flex justify-center gap-6 mt-6">
                   <button onClick={() => handleSwipe('left')} className="w-14 h-14 rounded-full bg-white border border-gray-200 text-red-500 shadow-lg flex items-center justify-center hover:bg-red-50 hover:scale-110 transition-all"><svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
                   <button onClick={() => handleSwipe('right')} className="w-14 h-14 rounded-full bg-white border border-gray-200 text-green-500 shadow-lg flex items-center justify-center hover:bg-green-50 hover:scale-110 transition-all"><svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg></button>
                </div>
              )}
            </div>
          )}
              ...
              <div className="space-y-6">
                 <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                       {CUISINE_OPTIONS.map(c => <button key={c.id} onClick={() => toggleCuisine(c.id)} className={`p-3 rounded-xl border text-sm font-medium flex items-center gap-2 transition-all ${prefs.cuisines.includes(c.id) ? 'bg-indigo-50 border-indigo-500 text-indigo-700 shadow-sm' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}><span>{c.icon}</span>{c.label}</button>)}
                    </div>
                 </div>
                 <div>
                    <label className="block text-sm font-semibold text-gray-600 mb-3">Exploration Style</label>
                    <div className="bg-white border border-gray-200 rounded-xl p-1 flex">
                        <button onClick={() => setPrefs({...prefs, explorationStyle: 'tourist'})} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${prefs.explorationStyle === 'tourist' ? 'bg-indigo-600 text-white shadow' : 'text-gray-500 hover:bg-gray-50'}`}>Must-Sees</button>
                        <button onClick={() => setPrefs({...prefs, explorationStyle: 'balanced'})} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${prefs.explorationStyle === 'balanced' ? 'bg-indigo-600 text-white shadow' : 'text-gray-500 hover:bg-gray-50'}`}>Balanced</button>
                        <button onClick={() => setPrefs({...prefs, explorationStyle: 'hidden_gem'})} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${prefs.explorationStyle === 'hidden_gem' ? 'bg-indigo-600 text-white shadow' : 'text-gray-500 hover:bg-gray-50'}`}>Hidden Gems</button>
                    </div>
                 </div>
              </div>
            </div>
          ...
          {step === 4 && (
            <div className="flex-1 animate-fade-in space-y-6">
               ...
               <div>
                ...
                <div className="grid grid-cols-3 gap-2">{['relaxed', 'moderate', 'fast'].map(t => <button key={t} onClick={() => setPrefs({...prefs, tempo: t as any})} className={`p-2 rounded-lg border text-sm capitalize transition-colors ${prefs.tempo === t ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}>{t}</button>)}</div>
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-600 mb-2">Budget</label>
                <div className="grid grid-cols-3 gap-2">{['budget', 'standard', 'luxury'].map(b => <button key={b} onClick={() => setPrefs({...prefs, budget: b as any})} className={`p-2 rounded-lg border text-sm capitalize transition-colors ${prefs.budget === b ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}>{b}</button>)}</div>
              </div>
               <div>
                   <label className="block text-sm font-semibold text-gray-600 mb-2">Transport Preference</label>
                   <div className="space-y-3">
                       {['public', 'rideshare', 'private'].map(tr => <button key={tr} onClick={() => setPrefs({...prefs, transport: tr as any})} className={`w-full p-3 rounded-xl border flex items-center gap-4 transition-all ${prefs.transport === tr ? 'bg-indigo-50 border-indigo-600 ring-1 ring-indigo-600' : 'bg-white border-gray-200 hover:bg-gray-50'}`}><div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 text-lg">{tr === 'public' ? '🚇' : tr === 'rideshare' ? '🚖' : '🚗'}</div><div className="text-left flex-1 font-bold text-gray-800 text-sm">{tr}</div></button>)}
                   </div>
               </div>
            </div>
          )}
          <div className="mt-8 flex justify-end pt-4 border-t border-gray-100">
             {step !== 2 && step > 1 && <button onClick={handleBack} className="px-6 py-3 text-gray-600 font-semibold hover:bg-gray-100 rounded-xl transition-colors mr-2">Back</button>}
             {(step !== 2 || currentCardIndex >= INTEREST_CARDS.length) && <button onClick={handleNext} disabled={step === 1 && (!startDate || !endDate)} className={`px-6 py-3 rounded-xl font-semibold transition-colors shadow-lg flex items-center ${step === 1 && (!startDate || !endDate) ? 'bg-gray-300 text-gray-500 cursor-not-allowed shadow-none' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-indigo-200'}`}>{step === TOTAL_STEPS ? 'Create Plan' : 'Continue'}</button>}
          </div>
        </div>
      </div>
    </div>
