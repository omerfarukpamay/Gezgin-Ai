...
export const TourGuideInterface: React.FC<TourGuideInterfaceProps> = ({ 
  ...
  useEffect(() => {
    if (currentStop) {
      ...
      const navMessageId = 'nav-' + currentStop.id;

      // Check if this navigation message already exists to prevent duplication on re-renders
      setMessages(prev => {
        if (prev.find(m => m.id === navMessageId)) return prev;
        return [...prev, {
          id: navMessageId,
          role: 'model',
          text: introText,
  ...
  const togglePlayPause = () => {
    if (isPaused) { synthRef.current.resume(); setIsPaused(false); setIsSpeaking(true); }
    else { synthRef.current.pause(); setIsPaused(true); setIsSpeaking(false); }
  };

  const replay = () => { if (currentAudioText) speak(currentAudioText, playbackSpeed); };
  const toggleSpeed = () => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => handleSend("Analyze this image given my current location.", reader.result as string);
        ...
        <div className="text-center">
           ...
           <div className="flex items-center justify-center gap-2 mt-1">
             <span className="text-xs px-2 py-0.5 bg-gray-800 rounded-full border border-gray-600 flex items-center gap-1">
                 ...
                 <span className="uppercase tracking-wide text-[10px]">{currentStop?.type ? `${currentStop.type} Mode` : 'Explorer Mode'}</span>
             </span>
           </div>
        </div>
      ...
      {(currentAudioText || isSpeaking) && (
          <div className="absolute top-20 left-0 right-0 z-40 flex justify-center animate-slide-down">
             <div className="bg-gray-800/95 backdrop-blur-md px-4 py-2 rounded-full flex gap-4 items-center shadow-2xl border border-indigo-500/30">
                <button onClick={replay} className="text-gray-300 hover:text-white">↻</button>
                <button onClick={togglePlayPause} className="text-white hover:text-indigo-400 transition transform hover:scale-110">
                  {isPaused || !isSpeaking ? <svg className="w-8 h-8 fill-current" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg> : <svg className="w-8 h-8 fill-current" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>}
                </button>
                <button onClick={toggleSpeed} className="text-xs font-mono font-bold bg-gray-700 px-2 py-1 rounded border border-gray-600 hover:bg-gray-600 transition min-w-[3rem]">{playbackSpeed}x</button>
      ...
      {arrivalState === 'approaching' && userLocation.name === currentStop?.activity && (
         <div className="absolute top-40 left-4 right-4 z-30 animate-bounce-in">
            <div className="bg-white text-gray-900 p-6 rounded-3xl shadow-2xl border-2 border-indigo-500 text-center">
               ...
               <button onClick={confirmArrival} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold shadow-lg shadow-indigo-200">Check In & Start Tour</button>
            </div>
         </div>
      )}
      <div className="flex-1 overflow-y-auto pt-36 pb-48 px-4 space-y-6 scrollbar-hide relative z-10">
        {messages.map(msg => (
          <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
            ...
            {msg.role === 'model' && (
              <button onClick={() => speak(msg.text, playbackSpeed)} className={`mt-2 ml-1 flex items-center gap-2 text-xs font-medium px-3 py-1.5 rounded-full border transition-all ${currentAudioText === msg.text ? 'bg-indigo-600 text-white border-indigo-500 ring-2 ring-indigo-400/30' : 'bg-gray-800 text-gray-300 border-gray-600 hover:bg-gray-700 hover:text-white'}`}>
                 {currentAudioText === msg.text && isSpeaking && !isPaused ? <div className="flex gap-0.5 h-3 items-end"><span className="w-0.5 bg-white h-2 animate-[bounce_0.5s_infinite]"></span><span className="w-0.5 bg-white h-3 animate-[bounce_0.7s_infinite]"></span><span className="w-0.5 bg-white h-1 animate-[bounce_0.9s_infinite]"></span></div> : <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" /></svg>}
        ...
        {isListening && <div className="flex justify-center items-center py-4 space-x-1"><div className="w-1 h-3 bg-red-500 rounded-full animate-pulse" style={{ animationDuration: '0.5s' }}></div><div className="w-1 h-5 bg-red-500 rounded-full animate-pulse" style={{ animationDuration: '0.3s' }}></div><div className="w-1 h-3 bg-red-500 rounded-full animate-pulse" style={{ animationDuration: '0.5s' }}></div><span className="text-red-400 text-xs ml-2">Listening...</span></div>}
        <div ref={scrollRef} />
      </div>
          ...
          <>
            <div className="flex gap-2 mb-4 overflow-x-auto scrollbar-hide">
               {['History?', 'Photo Spot?', 'Food nearby?'].map(chip => (
                 <button key={chip} onClick={() => handleSend(chip)} className="whitespace-nowrap px-4 py-2 bg-gray-800 rounded-full text-xs font-semibold border border-gray-700 hover:bg-indigo-900/50">{chip}</button>
               ))}
               <button onClick={goToNextStop} className="whitespace-nowrap px-4 py-2 bg-indigo-600 text-white rounded-full text-xs font-semibold">Complete & Next →</button>
            </div>
            <div className="flex items-center gap-3">
               <button onClick={() => fileInputRef.current?.click()} className="h-12 w-12 bg-gray-800 rounded-full flex items-center justify-center border border-gray-700 hover:bg-gray-700 transition-colors"><svg className="w-6 h-6 text-pink-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" /></svg></button>
               <button onClick={toggleMic} className={`h-12 w-12 rounded-full flex items-center justify-center border transition-all ${isListening ? 'bg-red-500 border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.5)]' : 'bg-gray-800 border-gray-700 hover:bg-gray-700'}`}><svg className={`w-6 h-6 ${isListening ? 'text-white' : 'text-blue-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg></button>
               <input type="text" className="flex-1 bg-gray-800 border-none rounded-full px-5 py-3 text-white focus:ring-2 focus:ring-indigo-500 placeholder-gray-500" placeholder="Talk or type..." value={inputText} onChange={e => setInputText(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSend(inputText)} />
               <button onClick={() => handleSend(inputText)} className="w-12 h-12 bg-indigo-600 rounded-full flex items-center justify-center shadow-lg hover:scale-105 transition-transform"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 10l7-7m0 0l7 7m-7-7v18" /></svg></button>
            </div>
          </>
